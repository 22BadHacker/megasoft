...
import {story} from '../Constants/main'
        ...
        <div className="w-full grid grid-cols-3 gap-[40px] ">
            {
                ...
                {item.icon}
                <p className='text-[28px] font-semibold text-white relative top-1 tracking-[.5px]'>{item.title}</p>
                <h5 className='text-[19px] text-white/80'>{item.desc}</h5>
            </div>)
            }
        </div> 

        <div className="flex gap-6 items-center pt-10">
            <Link to='/contact' className='text-white text-[16.5px] font-semibold bg-orangeLight py-2 px-4 rounded-[20px]'>Work With Us</Link>
            <Link to='/OurWork' className='text-orangee text-[16.5px] font-semibold border-[.5px] border-orangee py-2 px-4 rounded-[20px]'>Explore Our Work</Link>
        </div>
